...
**TODO:** _aggregated dataset statistics_
    ...
    render_ann_halltype = col2.checkbox('Visualize annotation type disagreement', value=False)
    ...
    for (g_model, g_summary), g_annotations in selected_faithfulness[selected_faithfulness.system != "Gold"].sort_values("system").groupby(['system', 'summary']):
        ...
        factuality_score = round(df_factuality.loc[g_model].mean_worker_factuality_score, 2) if g_model in df_factuality.index else "Missing"
        col1.write(f"""
**{g_model}**  
